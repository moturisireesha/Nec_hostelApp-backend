...
const sendSMS = async (phoneNumber, templateId, messageTemplate, variables) => {
    ...
    const url = `http://onlinebulksmslogin.com/v3/api.php?username=${encodeURIComponent(username)}&apikey=${encodeURIComponent(apiKey)}&senderid=${encodeURIComponent(senderId)}&mobile=${encodeURIComponent(phoneNumber)}&message=${encodeURIComponent(message)}&templateid=${encodeURIComponent(templateId)}`;
