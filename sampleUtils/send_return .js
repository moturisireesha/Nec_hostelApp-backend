...
const send_return = async (phoneNumber, messageTemplate, variables) => {
    ...
    const smsUrl = `http://onlinebulksmslogin.com/v3/api.php?username=${encodeURIComponent(username)}&apikey=${encodeURIComponent(apiKey)}&senderid=${encodeURIComponent(senderId)}&mobile=${encodeURIComponent(phoneNumber)}&message=${encodeURIComponent(message)}&templateid=${encodeURIComponent(return_id)}`;
